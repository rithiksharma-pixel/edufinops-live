...
async function loadSettings() { const { data, error } = await supabase.from('document_types').select('name,applies_to,is_required').eq('is_deleted', false).order('sequence_order'); if (error) throw error; $('documentTypesList').innerHTML = data.map((type) => `<div class="simple-row"><strong>${esc(type.name)}</strong><div class="muted">${esc(type.applies_to)}${type.is_required ? ' · Required' : ''}</div></div>`).join('') || '<p class="muted">No types configured.</p>'; }
