...
> in `052_external_portal_hardening_migration.sql` plus the portal code
...
> `053_external_portal_reporting_migration.sql`.
>
> **APPLIED to the live project on 2026-08-12** — 050, 052 and 053 are in.
> Re-verified afterwards by impersonation; results in "Verification after the
> fixes" below. 051 (the disbursement-date fix) is still unapplied and
> remains optional.
...
|---|---|---|---|---|
| 1 | Lenders can read the entire consultancy/BD channel roster | **High** — commercial data leak | Lender | Fixed in 052 |
| 2 | Consultants can read the entire consultancy/BD channel roster | **High** — commercial data leak | Consultant | Fixed in 052 |
| 3 | Every student name and loan amount renders blank in the Lender portal | **High** — app is broken | Lender | Fixed in 052 + `lenderDealService.js` |
| 4 | Lenders cannot resolve any internal staff name | Medium | Lender | Fixed in 052 + `lenderDealService.js` |
| 5 | Aadhaar / PAN / passport shipped to lenders unmasked | Medium — review, may be intended | Lender | **Open — your call** |
| 6 | Consultant portal has no document upload | Medium — functional gap | Consultant | Fixed in 052 + new `documentService.js` |
| 7 | Consultant cannot see lender-stage progress on their own student | Low — by design, worth revisiting | Consultant | Fixed in 052 + new drawer tab |
...
**Fix as built (052):** `v_lender_deal_list` — a narrow view exposing only
...
Re-run under impersonation, in a rolled-back transaction, with 052 applied:
...
## Addendum — a finding I got wrong, and the correction

I originally recorded here that internal RLS "is not scoped the way the
runbook claims", citing that an RM with 937 assigned leads can read all
12,190, and a Manager reads the same 12,277 rows as an Admin.

**The measurement was right. The conclusion was wrong.**

`main` carries `035_internal_staff_full_lead_access_migration.sql` and
`038_can_view_lead_follows_internal_staff_migration.sql`. Open internal
visibility is a deliberate product decision, made and migrated on purpose.
It is not an oversight, and there is nothing to fix.

I reached the wrong conclusion because I audited a branch that was 24
commits behind `main` and never checked. The stale `DEPLOYMENT.md` on that
branch still said "RM sees only their leads, Manager sees their team", so
the database disagreeing with it looked like a bug rather than like
documentation that had been superseded.

Recorded rather than deleted because the failure mode is worth keeping: an
audit is only as good as the base it runs against, and "the code disagrees
with the docs" is not evidence of a bug until you have checked which one is
older.
