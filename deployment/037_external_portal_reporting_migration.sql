...
-- Depends on 036 (which defines the access pattern these reuse).
...
-- described in the 036 header. The milestone definitions are kept
...
-- 034 and 035: nothing populates that column, so v_stage_milestones
...
comment on view v_lender_milestones is
  'Login/Sanction/PF Paid/Disbursement for the caller''s own bank. SECURITY DEFINER + security_barrier by design — see 036 header. Mirrors v_stage_milestones, which returns nothing for lenders.';
